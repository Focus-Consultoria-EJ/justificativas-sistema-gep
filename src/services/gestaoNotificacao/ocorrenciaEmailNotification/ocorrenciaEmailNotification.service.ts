...
class ocorrenciaEmailNotification extends emailService
{
    ...
    /**
     * Cria o template html enviado por e-mail quando uma ocorrência é deletada.
     * @param shark - objeto do tipo shark.
     * @param ocorrencia - objeto do tipo ocorrencia.
     * @param dataOcorrido - objeto do tipo Date ou um string no formato de data. Se não for passado pega automaticamente o dia atual. 
     * @returns retorna um string no formato html.
     */
    notificationEmailOnDelete(shark: Shark, ocorrencia: Ocorrencia, dataOcorrido?: Date)
    {
        const emailAntigo = {
            tipoOcorrencia: ocorrencia.tipoOcorrencia.nome!.toLowerCase(),
            metragemAtual: shark.metragem!
        };

        const restituicao = {
            metragemAnterior: shark.metragem!,
            metragemAtual: 0
        };
        ...
        if(emailAntigo.tipoOcorrencia.includes("aviso"))
        {
            if(ocorrencia.valorMetragem === 0)
                emailAntigo.tipoOcorrencia = "Aviso Comum";
            else if(ocorrencia.valorMetragem > 0)
            {
                emailAntigo.tipoOcorrencia = "Segundo aviso";
                emailAntigo.metragemAtual -= ocorrencia.valorMetragem;
                linhaAtualizacao = "Metragem a perder com o ";
            }
        ...
        if(emailAntigo.tipoOcorrencia.includes("gratifica"))
        {
            emailAntigo.metragemAtual += ocorrencia.valorMetragem;
            linhaAtualizacao = "Metragem a ganhar com a ";
            
        }
        else if(emailAntigo.tipoOcorrencia.includes("advert"))
        {
            emailAntigo.metragemAtual -= ocorrencia.valorMetragem;
            linhaAtualizacao = "Metragem a perder com a ";
        }

        restituicao.metragemAtual = shark.metragem!;
        restituicao.metragemAnterior = emailAntigo.metragemAtual;
            
        // Define o primeiro caractere para maiúsculo
        emailAntigo.tipoOcorrencia = emailAntigo.tipoOcorrencia.charAt(0).toUpperCase() + emailAntigo.tipoOcorrencia.slice(1);
        this.subject = "Remoção de Ocorrência";
        ...
        const html = `
            ...
            <div class="wrapper" style="color: black; max-width: 700px; margin: 0 auto; height: 100vh;">
                <p> 
                    Olá, shark ${shark.nome}. Como você está? <br>
                    O GEP vem por meio desse e-mail, informar que você removemos a ocorrência enviada anteriormente. 
                    Quaisquer metros perdidos ou ganhos foram restituídos ao estado anterior.

                    <ul>
                        <li><b>Situação anterior do Shark</b>:  ${restituicao.metragemAnterior}</li>
                        <li><b>Situação atual do Shark</b>:  ${restituicao.metragemAtual}</li>
                    </ul>
                </p>

                <div>
                    <h3>Notificação anterior que foi removida: </h3>
                    <div class="old-email" style="background-color: #DCC8B3; border: 2px solid #C7A37E; padding: 20px;">
                        <h2>${emailAntigo.tipoOcorrencia}</h2>
                        <p>
                            Olá, shark ${shark.nome}. Como você está? <br>
                            O GEP vem por meio desse e-mail, informar que você está recebendo ${artigo} <mark>${emailAntigo.tipoOcorrencia}<mark>.
                            <ul>
                                <li><b>Motivo</b>: ${ocorrencia.mensagem}</li>
                                <li><b>Na data</b>: ${dataOcorrido ? dataFormatToBR(dataOcorrido!) :  dataFormatToBR(new Date(ocorrencia.dataOcorrido))}</li>
                                <li><b>Situação anterior do Shark</b>:  ${shark.metragem}</li>
                                <li><b>${linhaAtualizacao}<mark>${emailAntigo.tipoOcorrencia}</mark></b>:  ${ocorrencia.valorMetragem}</li>
                                <li><b>Situação atual do Shark</b>:  ${emailAntigo.metragemAtual}</li>
                                <li><b>Sistema de metragem</b>: Conversa com o GEP, 14 metros; Conversa com a DE, 8 metros;
                                Desligamento, 0 metros.</li>
                                <li><b>Sistema de avisos</b>: O shark poderá recerber inúmeros avisos, porém sendo contabilizado cada um como "aviso comum" ou "segundo aviso". Após ter recebido o 2º aviso, a contagem irá zerar, então o próximo irá ser um "aviso comum" e em seguida "segundo aviso", e assim sucessivamente. 
                                <u>No 2º aviso o shark irá perder 2 metros.</u></li>
