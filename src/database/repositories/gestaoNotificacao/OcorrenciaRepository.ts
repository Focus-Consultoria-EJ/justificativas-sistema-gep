...
import {
  LogOcorrencia,
  Ocorrencia,
} from "../../../models/gestaoNotificacao/Ocorrencia";
import { TableNames } from "../../TableNames";
import db from "../../db";
import {
  membroAtivoParameter,
  orderByParameter,
  pageParameter,
  sizeParameter,
} from "../../Parameters";
...
class OcorrenciaRepository {
  ...
  removeSelectedNullAttributes(ocorrencia: Ocorrencia) {
    ...
    if (ocorrencia.uploadFile?.id === null) delete ocorrencia.uploadFile;
  ...
  async select(
    ...
    page?: number,
    membroAtivo?: string,
    nomeSharkCriador?: string,
    nomeSharkReferente?: string,
    emailSharkCriador?: string,
    emailSharkReferente?: string,
    ...
    try {
      ...
      let query = db(TableNames.shark)
        ...
        .innerJoin(
          `${TableNames.tipo_ocorrencia} as toc`,
          "oc.id_tipo_ocorrencia",
          "toc.id"
        )
        .innerJoin(
          `${TableNames.tipo_assunto} as tas`,
          "oc.id_tipo_assunto",
          "tas.id"
        )
        ...
        .innerJoin(
          `${TableNames.shark} as sr`,
          "oc.id_shark_referente",
          "sr.id"
        )
        .innerJoin(`${TableNames.celula} as src`, "sr.id_celula", "src.id")
        .leftJoin(
          `${TableNames.nivel_gratificacao} as nvg`,
          "oc.id_nivel_gratificacao",
          "nvg.id"
        )
        .leftJoin(
          `${TableNames.nivel_advertencia} as nva`,
          "oc.id_nivel_advertencia",
          "nva.id"
        )
        .leftJoin(
          `${TableNames.upload_file} as upf`,
          "oc.id",
          "upf.id_ocorrencia"
        )
      ...
      if (nomeSharkCriador)
        query = query.andWhere("sc.nome", "ilike", `%${nomeSharkCriador}%`);
      if (nomeSharkReferente)
        query = query.andWhere("sr.nome", "ilike", `%${nomeSharkReferente}%`);
      if (emailSharkCriador)
        query = query.andWhere("sc.email", "ilike", `%${emailSharkCriador}%`);
      if (emailSharkReferente)
        query = query.andWhere("sr.email", "ilike", `%${emailSharkReferente}%`);
      if (tipoOcorrencia)
        query = query.andWhere("toc.nome", "ilike", `%${tipoOcorrencia}%`);
      if (tipoAssunto)
        query = query.andWhere("tas.nome", "ilike", `%${tipoAssunto}%`);
      ...
      const ocorrencias: Ocorrencia[] = data.map((ocorrencia) => {
        const ocorrenciaEditada = {
          ...
          tipoOcorrencia: {
            id: ocorrencia.id_tipo_ocorrencia,
            nome: ocorrencia.tipo_ocorrencia,
          },
          tipoAssunto: {
            id: ocorrencia.id_tipo_assunto,
            nome: ocorrencia.tipo_assunto,
          },
          mensagem: ocorrencia.mensagem,
          valorMetragem: ocorrencia.valor_metragem,
          nivelGratificacao: {
            id: ocorrencia.id_nivel_gratificacao,
            nome: ocorrencia.nome_nivel_gratificacao,
            valor: ocorrencia.valor_nivel_gratificacao,
          },
          nivelAdvertenocia: {
            id: ocorrencia.id_nivel_advertencia,
            nome: ocorrencia.nome_nivel_advertencia,
            valor: ocorrencia.valor_nivel_advertencia,
          },
          sharkCriador: {
            id: ocorrencia.id_shark_criador,
            nome: ocorrencia.nome_shark_criador,
            email: ocorrencia.email_shark_criador,
            celula: {
              id: ocorrencia.id_celula_shark_criador,
              nome: ocorrencia.nome_celula_shark_criador,
            },
          },
          sharkReferente: {
            id: ocorrencia.id_shark_referente,
            nome: ocorrencia.nome_shark_referente,
            email: ocorrencia.email_shark_referente,
            celula: {
              id: ocorrencia.id_celula_shark_referente,
              nome: ocorrencia.nome_celula_shark_referente,
            },
          },
          uploadFile: {
            id: ocorrencia.id_updetad_file,
            googleDriveId: ocorrencia.id_google_drive,
            nomeArquivo: ocorrencia.nome_arquivo,
          },
  ...
  async getById(id: number): Promise<Ocorrencia | undefined> {
    try {
      ...
      const ocorrrencia = {
        ...
        tipoOcorrencia: {
          id: data.id_tipo_ocorrencia,
          nome: data.tipo_ocorrencia,
        },
        tipoAssunto: { id: data.id_tipo_assunto, nome: data.tipo_assunto },
        mensagem: data.mensagem,
        valorMetragem: data.valor_metragem,
        nivelGratificacao: {
          id: data.id_nivel_gratificacao,
          nome: data.nome_nivel_gratificacao,
          valor: data.valor_nivel_gratificacao,
        },
        nivelAdvertencia: {
          id: data.id_nivel_advertencia,
          nome: data.nome_nivel_advertencia,
          valor: data.valor_nivel_advertencia,
        },
        sharkCriador: {
          ...
          celula: {
            id: data.id_celula_shark_criador,
            nome: data.nome_celula_shark_criador,
          },
        },
        sharkReferente: {
          id: data.id_shark_referente,
          nome: data.nome_shark_referente,
          email: data.email_shark_referente,
          celula: {
            id: data.id_celula_shark_referente,
            nome: data.nome_celula_shark_referente,
          },
        },
        uploadFile: {
          id: data.id_updetad_file,
          googleDriveId: data.id_google_drive,
  ...
  async delete(id: number): Promise<any | undefined> {
    return await db(TableNames.ocorrencia).select().where({ id: id }).del();
  }
  ...
  async deleteAllOcorrencia(): Promise<any | undefined> {
    return await db(TableNames.ocorrencia).del().where("id", ">", 0);
  }
   ...
   * @returns uma promise contendo uma coleção de objetos.
   */
  async selectOcorrenciaLog(
    size?: number,
    page?: number,
    order?: string
  ): Promise<LogOcorrencia[] | undefined> {
    size = sizeParameter(size);
    page = pageParameter(page);
    order = orderByParameter(order, "DESC");
    ...
    let query = db(TableNames.ocorrencia_log)
      ...
      .innerJoin(
        `${TableNames.tipo_acao_log} as tal`,
        "tal.id",
        "ol.id_tipo_acao_log"
      )
   ...
   */
  async insertOcorrenciaLog(
    idTipoAcaoLog: number,
    id: number,
    idSharkEditor: number
  ): Promise<any | undefined> {
    return await db(TableNames.ocorrencia_log).insert({
  ...
  async deleteAllOcorrenciaLog(): Promise<any | undefined> {
    return await db(TableNames.ocorrencia_log).del().where("id", ">", 0);
  }
}
