...
const generateData = async (numRegistros: number) => {
    ...
    const dadosGerados: Dados[] = [];

    const dadosIniciais = { 
        nome: "João Paulo", 
        email: "emailtest1@hotmail.com", 
        senha: await passwordEncrypt("123456789"),
        matricula: "12345678910", 
        distancia: random(1, 3), 
        celula: random(1, 6),
        telefone: "22987654321",
        admin: random(0, 1),
        membro_ativo: random(0, 1)
    };
    ...
    for (let i = 0; i < numRegistros; i++)
    {
        ...
        dadosGerados.push({ nome, email, senha, matricula, id_distancia_residencia:distancia, id_celula: 
